...
namespace LeashFramework::UI::ModMenu {
    ...
    namespace {
        ...
        void ApplySettings(const ModMenuSettings& a_settings) {
            ...
            debugSettings = a_settings.debug;
        ...
        void RenderSkeletonDumper() {
            MenuLayout::Panel("Skeleton inspector", "Inspect skinned bones on the physical leash owner. The configured parent and matching rope bones are highlighted.", [] {
                ...
                MenuLayout::Field("Find a bone", [](const char* a_id) {
                    return ImGuiMCP::InputTextWithHint(a_id, "Filter bone names", skeletonFilter, sizeof(skeletonFilter));
                }, "Show matching bones and their ancestors. Clear the filter to see the complete snapshot.");
        ...
        void RenderArmorEntries() {
            ...
            for (std::size_t index = 0; index < debugSettings.armorEntries.size(); ++index) {
                ...
                if (editEntries) {
                    MenuLayout::Field("Plugin", [&](const char* a_id) { return ImGuiMCP::InputText(a_id, entry.modName, sizeof(entry.modName)); });
                    MenuLayout::Field("Local FormID #Name", [&](const char* a_id) { return ImGuiMCP::InputText(a_id, entry.formID, sizeof(entry.formID)); },
        ...
        void RenderAnchorSettings() {
            ...
            if (!debugSettings.holderOwnsLeash && selectedAnchorType == DebugAnchorType::kWorldPosition) {
                ...
                MenuLayout::Field(debugSettings.holderOwnsLeash ? "Bone on leashed actor" : "Bone on leasher",
                    [](const char* a_id) { return ImGuiMCP::InputText(a_id, selectedAttachmentBone, sizeof(selectedAttachmentBone)); }, "Exact name of the bone at the rope's free end.");
            ...
            MenuLayout::Columns(
                [] {
                    MenuLayout::Panel("Collision overlay", "Visualize the body shapes used for actor collision.", [] {
                        ...
                        MenuLayout::Help("Draw the actor collision shapes for debugging. This does not enable or disable physical collision.");
                ...
                [] {
                    MenuLayout::Panel("Pull diagnostics", "Record detailed pull-controller information in the plugin log.", [] {
                        ...
                        MenuLayout::Help("Enable detailed pull diagnostics. This preference is saved when the menu closes.");
